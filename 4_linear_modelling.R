...
## SCAMP SUBSET to F1 and have PHQ data
scamp <- psytools 

## Subset sample by have taken follow-up psytools
scamp_f1 <- scamp %>% 
  filter(!is.na(PSYTOOLS_UID_F1)) %>%
  filter(!is.na(SCHOOL_ID_F1))


## Create function to fit linear GAD models for BASELINE
fit_phq_lm_BL <- function(exposure_var, df, internet=FALSE, device_adjust=FALSE){
  
  device <- ifelse(internet==TRUE, "ownership_BL + internet_BL", "internet_BL")
  ...
  demog <- ifelse(factor == TRUE, paste(demog, " + {device}"), demog)
...
test_phq <- fit_phq_lm_BL("sns_mp_avg_BL", scamp_f1)
test <- fit_gad_lm_BL("internet_avg_BL", scamp_f1)

x <- jtools::summ(test_phq$model_all, confint=TRUE, conf.method = "profile")
y <- jtools::summ(test_phq$model_sleep, confint=TRUE, conf.method = "profile")

jtools::export_summs(test_phq$model_all, test_phq$model_sleep, error_format = "[{conf.low}, {conf.high}]", conf.method = "profile")
