...
## PHQ9 Proportional Odds Models

```{r phq_models_BL, echo=FALSE, results='asis'}
...
for(row in 1:nrow(df)) {
  ...
  if(!file.exists(paste0('./models/polr/phq_polr_', df$exposure_var[row], '.rds'))){
    ...
    phq_polr <- fit_phq_polr_BL(df$exposure_var[row], scamp_f1, internet = internet_option, device_adjust = TRUE)
    
    models <- list(
      "Exposure Only" = phq_polr$model_uni,
      "Demog" = phq_polr$model_demo,
      "Demog + SDQ BL" = phq_polr$model_sdq,
      "Demog + SDQ BL + Sleep BL" = phq_polr$model_sleep,
      "Demog + SDQ BL + PA" = phq_polr$model_phys
      )
    ...
    p <- jtools::plot_coefs(phq_polr$model_uni, phq_polr$model_demo, phq_polr$model_sdq, 
...
```{r gad_models_BL, echo=FALSE, results='asis'}
df <- exposure_df %>% filter(factor_continuous == "continuous" & timepoint == "BL")
...
for(row in 1:nrow(df)) { # ){
  ...
  if(!file.exists(paste0('./models/polr/gad_polr_', df$exposure_var[row], '.rds'))){
    ...
    gad_polr <- fit_gad_polr_BL(df$exposure_var[row], scamp_f1, internet = internet_option, device_adjust = TRUE)
    
    models <- list(
      "Exposure Only" = gad_polr$model_uni,
      "Demog" = gad_polr$model_demo,
      "Demog + SDQ BL" = gad_polr$model_sdq,
    ...
    p <- jtools::plot_coefs(gad_polr$model_uni, gad_polr$model_demo, gad_polr$model_sdq, 
    ...
    saveRDS(output_list, paste0('./models/polr/gad_polr_', df$exposure_var[row], '.rds'))
