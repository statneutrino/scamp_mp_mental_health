...
df <- exposure_df %>% filter(factor_continuous == "factor" & timepoint == "F1")
...
for(row in 1:nrow(df)) { # ){
  ...
  if(!file.exists(paste0('./models/phq_', df$exposure_var[row], '.rds'))){
    phq_model <- fit_phq_lm_F1(df$exposure_var[row], scamp_f1)
    
    table_summary <- jtools::export_summs(
        phq_model$model_uni_RE, phq_model$model_demo, 
    ...
    p <- plot_coefs(phq_model$model_uni_RE, phq_model$model_demo, phq_model$model_sleep, phq_model$model_phys, 
                    coefs = coef_names )
  ...
  if(!file.exists(paste0('./models/gad_', df$exposure_var[row], '.rds'))){
    gad_model <- fit_gad_lm_F1(df$exposure_var[row], scamp_f1)
    
    table_summary <- jtools::export_summs(
        gad_model$model_uni_RE, gad_model$model_demo,  
    ...
    coef_names <- paste0(df$exposure_var[row], levels(scamp_f1[,df$exposure_var[row]])[-1])
    names(coef_names) <- levels(scamp_f1[,df$exposure_var[row]])[-1]
    p <- plot_coefs(gad_model$model_uni_RE, gad_model$model_demo, gad_model$model_sleep, gad_model$model_phys, 
                    coefs = coef_names  )
...
for(row in 1:nrow(df)) { # ){
  ...
  if(!file.exists(paste0('./models/phq_', df$exposure_var[row], '.rds'))){
    ...
    phq_model <- fit_phq_lm_F1(df$exposure_var[row], scamp_f1, internet = internet_option, device_adjust = TRUE)
  ...
  if(!file.exists(paste0('./models/gad_', df$exposure_var[row], '.rds'))){
    ...
    gad_model <- fit_gad_lm_F1(df$exposure_var[row], scamp_f1, internet = internet_option, device_adjust = TRUE)
